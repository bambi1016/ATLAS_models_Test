...
def get_labels(test_path_root):
    ...
    test_data = DataLoader(CustomDataset2_gray(test_data_list, test_mask_list), batch_size=2, shuffle=True, num_workers=4)
...
def test(model, device, label_list_global):
    ...
    for i, pack in enumerate(test_data):
        ...
        img_scale = 189
        for img_i in range(img_scale):
            ## 189 장 이미지를 예측
            if img_i == (img_scale-1):  ## 없어두 뎀
...
if __name__ == '__main__':
    ...
    model_path = "UNet_in2d_out2dUnet_ATLAS_test_v1_Dice_IoULoss_1/Unet_ATLAS_test_best_model.pth"

    net = UNet(n_classes=2, n_channels=1).cuda()
    ...
    test_score = test(net, device, "label_list_global")
    print(test_score)
