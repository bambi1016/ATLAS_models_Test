...
def train():
    ...
    logdir = 'D:/soyeon_vision2022/01.Object_segmentation_start/03.code/old/ATLAS_models/D-UNet_in2d_out2d'+model_name+'_v1_Dice_IoULoss_'+str(img_depth)
    ...
    train_data = DataLoader(CustomDataset_3d_gray_out_1(train_data_list, train_mask_list, img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    val_data = DataLoader(CustomDataset_3d_gray_out_1(val_data_list, val_mask_list, img_h=192, img_w=192, out_num=img_depth_out_num), batch_size=batch_size, shuffle=True, num_workers=4)
    ...
    while start_iter <= num_epochs and flag:
        ...
        with torch.no_grad():
            for i_val, (images_val, labels_val) in tqdm(enumerate(val_data)):
                ...
                outputs = model(images_val)
                val_loss = criterion(outputs, labels_val.squeeze(1))
...
if __name__ == '__main__':
    ...
    test_score_list = []
    model_name = "D-UNet_ATLAS_test"
    logdir = 'D:/soyeon_vision2022/01.Object_segmentation_start/03.code/old/ATLAS_models/D-UNet_in2d_out2d' + model_name + '_v1_Dice_IoULoss_' + str(1)
